...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      videos: [],
      selectedVideo: {id: {videoId:"Fq2CvmgoO7I"}},
      data: [],
      caption: '',
    };
...
playCaptions=() =>{
  ...
  fetch(url)
        ...
        .then(data => {
          ...
          elem.innerHTML = data;
          data = elem.value;
          let j = JSON.parse(convert.xml2json(data, {compact: true, spaces: 4}));
          console.log(j);
          //const captions = j.transcript.text.map((caption)=>{caption._text})
          this.setState({data: j.transcript.text})
          j.transcript.text.map(caption => {
            console.log(caption);
            let timeOut = 
            setTimeout(function(){
              console.log(caption._text);
              this.setState({"caption": caption._text});
  ...
  render() {
    return (
      <div>
        ...
        <li>{translate.translate(this.state.caption)}</li>
